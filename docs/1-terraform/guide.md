...
| `make tunnel`: `TargetNotConnected`, and the gateway never appears in `aws ssm describe-instance-information` | The gateway lost a race with IAM propagation. Its instance profile is attached by the same apply that launches it, so the role can take minutes to reach the instance metadata service, while the SSM agent asks for credentials about **20 seconds** after boot. Having failed, the agent falls back to Default Host Management Configuration — an account-level feature this account does not use — and stays there, so the instance never registers and there is no way in to repair it | Read the reason rather than guessing, with `aws ec2 get-console-output --instance-id <id> \| grep -i ssm`: the line is `no valid credentials could be retrieved for ec2 identity`. `wireguard-init.sh` now waits for the role on IMDS and then restarts the agent, so a gateway built from current code recovers by itself. An instance already stuck this way cannot be repaired — replace it: `terraform -chdir=infra/terraform/cluster apply -replace=aws_instance.wireguard` (measured 2026-09-23) |
